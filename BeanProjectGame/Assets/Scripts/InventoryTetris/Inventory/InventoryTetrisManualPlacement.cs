...
public class InventoryTetrisManualPlacement : MonoBehaviour {
    ...
    private void Update() {
        ...
        if (Input.GetKeyDown(KeyCode.Alpha9)) { placedObjectTypeSO = placedObjectTypeSOList[8]; RefreshSelectedObjectType(); }
    ...
    public Vector2 GetCanvasSnappedPosition() {
        RectTransformUtility.ScreenPointToLocalPointInRectangle(itemContainer, Input.mousePosition, null, out Vector2 anchoredPosition);
        //RectTransformUtility.ScreenPointToWorldPointInRectangle(itemContainer, Input.mousePosition, gameCamera, out Vector3 anchoredPosition);
        //anchoredPosition *= canvas.scaleFactor;
